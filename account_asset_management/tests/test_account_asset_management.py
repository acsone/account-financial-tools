...
class TestAssetManagement(common.TransactionCase):
    ...
    def test_residual_value_zero_with_profit(self):
        ...
        self.assertTrue(asset.account_move_line_ids.filtered(lambda l: l.debit == sale_value and l.account_id == account_sale))
        self.assertTrue(asset.account_move_line_ids.filtered(lambda l: l.credit == sale_value and l.account_id == account_plus_value))
        # Ensure others move lines still here with corrects amount
        self.assertTrue(asset.account_move_line_ids.filtered(lambda l: l.credit == asset.purchase_value))
        self.assertTrue(asset.account_move_line_ids.filtered(lambda l: l.credit == asset.purchase_value))
        return
