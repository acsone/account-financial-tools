...
class AssetDepreciationConfirmationWizard(models.TransientModel):
    ...
    def asset_compute(self):
        ...
        if self.env.context.get('not_async'):
            return super(AssetDepreciationConfirmationWizard, self.with_context(asset_batch_processing=True)).asset_compute()
        else:
            session = ConnectorSession.from_env(self.env)
            description = _("Creating jobs to create moves for assets period %s") % (self.period_id.id,)
            async_asset_compute.delay(session, self.period_id.id, description=description)
