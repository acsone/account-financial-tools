...
class TestAccountMoveFiscalMonth(TransactionCase):
    ...
    def test_01_account_move_date_range_id_compute(self):
        january_1st = Date.from_string('2017-01-01')
        move = self.create_account_move(january_1st)

        self.assertEquals(
            move.date_range_id, self.date_range_january_2017,
            msg="Move period should be January 2017")
        self.assertTrue(all([
            line.date_range_id == self.date_range_january_2017
        ...
        self.assertFalse(
            bool(move.date_range_id),
            msg="Move shouldn't have any date range")

    def test_02_account_move_date_range_id_search(self):
        ...
        moves = self.AccountMoveObj.search([
            ('date_range_id', '=', self.date_range_january_2017.id),
        ])
