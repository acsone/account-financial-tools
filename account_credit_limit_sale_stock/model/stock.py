    ...
    def do_transfer(self, cr, uid, ids, context=None):
        ...
        if context.get('force', False) is not True:
            ...
            stock_move_ids = stock_move_model\
                .search(cr,
                        uid,
                        [('picking_id.id', '=', stock_picking_obj.id),
            ...
            if stock_picking_obj.partner_id.commercial_partner_id.level_amount\
                ...
                amount = amount_order
            diff = stock_picking_obj.partner_id\
                .commercial_partner_id.credit_limit - \
                stock_picking_obj.partner_id.commercial_partner_id.level_amount
            if (diff > 0):
                amount = amount - diff
            self.pool.get('force.tracking')\
                .create(cr,
                        ...
                        {'user_id': uid,
                         'type': 'delivery_order',
                         'source_document': stock_picking_obj.name,
...
class warning_force_picking_wizard(orm.TransientModel):
    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        transfer_detail_id = context.get('transfer_detail_id')
        return self.pool.get('stock.transfer_details')\
            .do_detailed_transfer(cr,
                                  uid,
                                  transfer_detail_id,
                                  context=context)
